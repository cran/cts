...
G_y(f)=\frac{\sigma^2/\kappa^{2p-2}}{\kappa^2 + (2\pi f)^2} \left| \frac{\phi(-1)}{\phi(2\pi i g)}\right|^2 2\pi,
\end{equation*}
where 
\begin{equation*}
\phi(z) = 1-\phi_1z - ... - \phi_p z^p.
\end{equation*}
The new parameter space is identical to that of the stationary discrete time autoregressive
...
### decompose the original time series into three corresponding components via the Kalman smoother as shown in Figure~\ref{fig:asth3}. 
...
\centering
<<fig=true>>=
### predict the last 10 steps past the end of time series in Figure~\ref{fig:asth4}.
...
%The author thanks Granville Tunnicliffe Wilson for invaluable assistance.
%The author also thanks the Editors and reviewers for their constructive remarks and suggestions, which have greatly improved the article and software. 
\bibliography{kf}
