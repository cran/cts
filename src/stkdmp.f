...
C
      IF (INIT) CALL I0TK00(INIT, 5000, 4)
C
C
...
C
      ERROUT = I1MACH(4)
c      WRITE (ERROUT,  9901)
c 9901   FORMAT (11H1STACK DUMP)
      call rwarn('STACK DUMP')
C
C
...
C
c         WRITE (ERROUT,  9902)
c 9902      FORMAT (29H0STACK HEADING IS OVERWRITTEN)
c         WRITE (ERROUT,  9903)
c 9903      FORMAT (47H UNSTRUCTURED DUMP OF THE DEFAULT STACK FOLLOWS)
         call rwarn('HEADING IS OVERWRITTEN')
         call rwarn('UNSTRUCTURED DUMP OF THE DEFAULT STACK FOLLOWS')
C
...
C
   10    call rwarn('STORAGE PARAMETERS')
C   10    WRITE (ERROUT,  9904)
C 9904      FORMAT (19H0STORAGE PARAMETERS)
         call intpr('LOGICAL', -1, ISIZE(1), 1)
         call intpr('INTEGER', -1, ISIZE(2), 1)
         call intpr('REAL', -1, ISIZE(3), 1)
         call intpr('DOUBLE PRECISION', -1, ISIZE(4), 1)
         call intpr('COMPLEX', -1, ISIZE(5), 1)
C         WRITE (ERROUT,  9905) ISIZE(1)
C 9905      FORMAT (18H LOGICAL          , I7, 14H STORAGE UNITS)
C         WRITE (ERROUT,  9906) ISIZE(2)
C 9906      FORMAT (18H INTEGER          , I7, 14H STORAGE UNITS)
C         WRITE (ERROUT,  9907) ISIZE(3)
C 9907      FORMAT (18H REAL             , I7, 14H STORAGE UNITS)
C         WRITE (ERROUT,  9908) ISIZE(4)
C 9908      FORMAT (18H DOUBLE PRECISION , I7, 14H STORAGE UNITS)
C         WRITE (ERROUT,  9909) ISIZE(5)
C 9909      FORMAT (18H COMPLEX          , I7, 14H STORAGE UNITS)

C
C  WRITE OUT THE CURRENT STACK STATISTICS (I.E. USAGE)
C
         call intpr('STACK STATISTICS', -1, 1, 0)
         call intpr('STACK SIZE', -1, LMAX, 1)
         call intpr('MAXIMUM STACK USED', -1, LUSED,1)
         call intpr('CURRENT STACK USED', -1, LNOW,1)
         call intpr('NUMBER OF ALLOCATIONS',-1, LOUT,1)
c         WRITE (ERROUT,  9910)
c 9910      FORMAT (17H0STACK STATISTICS)
c         WRITE (ERROUT,  9911) LMAX
c 9911      FORMAT (23H STACK SIZE            , I7)
c         WRITE (ERROUT,  9912) LUSED
c 9912      FORMAT (23H MAXIMUM STACK USED    , I7)
c         WRITE (ERROUT,  9913) LNOW
c 9913      FORMAT (23H CURRENT STACK USED    , I7)
c         WRITE (ERROUT,  9914) LOUT
c 9914      FORMAT (23H NUMBER OF ALLOCATIONS , I7)
C
...
C
              call rwarn('STACK PARTIALLY OVERWRITTEN')
              call rwarn('UNSTRUCTURED DUMP OF REMAINING STACK FOLLOWS')
C               WRITE (ERROUT,  9915)
C 9915            FORMAT (28H0STACK PARTIALLY OVERWRITTEN)
C               WRITE (ERROUT,  9916)
C 9916          FORMAT (45H UNSTRUCTURED DUMP OF REMAINING STACK FOLLOWS)
C
         DO  30 I = 1, 5
...
C
 100        BPNTR = LNEXT
            ...
            GO TO 20
C
  110  call rwarn('END OF STACK DUMP')
C  110  WRITE (ERROUT,  9922)
c 9922   FORMAT (18H0END OF STACK DUMP)
      RETURN
      END
